...
export default function MealPage() {
    const today = new Date("2023-09-03");
    return (
...
function CalenderBody({ today }) {
    ...
    const monthString = (month) => {
        //해당 날짜의 달을 문자열로 변환
        if (month + 1 < 10) return "0" + (month + 1); //3월은 03 형태로
        else return (month + 1).toString();
    };

    const dateString = (date) => {
        //해당 날짜의 일을 문자열로 변환
        if (date < 10) return "0" + date; //9일은 09 형태로
                ...
                >
                    ...
                    <div className={styles.todayMeal}>
                        <MealList
                            ...
                            FROM_YMD={
                                year.toString() +
                                monthString(monthStart.getMonth()) +
                                dateString(monthStart.getDate())
                            }
                            TO_YMD={
                                year.toString() +
                                monthString(monthEnd.getMonth()) +
                                dateString(monthEnd.getDate())
                            }
                            dateOfToday={
                                day.getFullYear().toString() +
                                monthString(day.getMonth()) +
                                dateString(day.getDate())
                            }
                            name="meal-page"
                        />
                    </div>
